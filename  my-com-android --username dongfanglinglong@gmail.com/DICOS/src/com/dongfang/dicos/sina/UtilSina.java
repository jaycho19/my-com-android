...
public class UtilSina {
	...
	private static final String			tag						= "UtilSina";
	...
	public static String post(Context context, String url, String msg, String key, String token) throws WeiboException {
		...
		try {
			...
			post.setHeader("source", key);
			post.setHeader("access_token", token);
			post.setHeader("status", msg);
			...
			ULog.d(tag, "statusCode = " + statusCode);
			if (statusCode != 200) {
	...
	public static String encodeParameters(WeiboParameters httpParams) {
		if (null == httpParams || UtilSina.isBundleEmpty(httpParams)) {
			return "";
		}
