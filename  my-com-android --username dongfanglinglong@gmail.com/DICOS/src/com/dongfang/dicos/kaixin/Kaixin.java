...
import com.dongfang.dicos.more.KaixinWeiboAuthorization;
...
public class Kaixin {
	 ...
	 */
	public static final String	API_KEY								= ComParams.KAIXIN_APP_KEY;						// 替换为申请时获得的api
																														// key

	/**
	 * 组件申请时获得的secret key
	 */
	public static final String	SECRET_KEY							= ComParams.KAIXIN_APP_SECRET;						// 替换为申请时获得的secret
																														// key

	/**
	 * 组件申请时填写的网站地址
	 */
	private static String		KX_AUTHORIZE_CALLBACK_URL			= ComParams.KAIXIN_APP_URL;						// 替换为申请组件时填写的网站地址
	...
	private KaixinAuthListener	listener							= null;

	public KaixinAuthListener getKaixinAuthListener() {
		return listener;
	}
	...
	public void authorizeByActivity(final Context context, String[] permissions, final KaixinAuthListener listener) {
		if (this.isSessionValid()) {
			listener.onAuthComplete(new Bundle());
			return;
		}
		...
		params.putString("redirect_uri", KX_AUTHORIZE_CALLBACK_URL);
		...
		if (context.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			UtilKaiXin.showAlert(context, "没有权限", "应用需要访问互联网的权限");
		} else {
				...
				private int parseUrl(String url) {
					if (url.startsWith(KX_AUTHORIZE_CALLBACK_URL)) {
						...
						if (error != null) {
							...
							UtilKaiXin.clearCookies(context);
	...
	public String refreshAccessToken(Context context, String[] permissions) throws FileNotFoundException,
		...
		return UtilKaiXin.openUrl(context, KX_REFRESHTOKEN_URL, "GET", params, null);
	}
	...
	public String uploadContent(Context context, String restInterface, Bundle params, Map<String, Object> photos)
		...
		ULog.d(tag, "uploadContent AccessToken = " + getAccessToken());
		params.putString("access_token", getAccessToken());

		return UtilKaiXin.openUrl(context, KX_REST_URL + restInterface, "POST", params, photos);
	}
	...
	public String request(Context context, String restInterface, Bundle params, String httpMethod)
		...
		params.putString("access_token", getAccessToken());
		return UtilKaiXin.openUrl(context, KX_REST_URL + restInterface, httpMethod, params, null);
	}
	...
	public String login(Bundle params, Context ctx) {
		...
		String sError = null;
		try {
			String response = UtilKaiXin.openUrl(ctx, url, method, params, null);
			if (response != null) {
	...
	public boolean loadStorage(Context context) {
		...
		if (accessToken == null || refreshToken == null) {
			...
			mRefreshToken = null;
			mAccessExpires = 0;
			ULog.d(tag, mAccessToken + " + " + mRefreshToken + " + " + mAccessExpires);
			return false;
		}
