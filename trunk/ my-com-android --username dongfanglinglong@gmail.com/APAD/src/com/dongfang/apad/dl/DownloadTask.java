...
import com.dongfang.apad.net.Http;
import com.dongfang.apad.param.Types;
...
public class DownloadTask extends AsyncTask<Void, DownloadInfo, Bundle> {
	...
	protected Bundle doInBackground(Void... params) {
		...
		try {
			...
			// DFException.DOWNLOAD_CONTENT_LENGTH_ZERO);
			// return bundle;
			// }
				...
				try {
							...
							if (!dlInfo.filePath.contains(path)) {
								...
								if (needSpace < tempFreeStorage) {
									if (Types.DOWNLOAD_TYPE_IMAGE == dlInfo.contentType)
										dlInfo.filePath = path + "/Android/data/" + context.getPackageName() + "/cache/" + dlInfo.fileName;
									else
										dlInfo.filePath = path + "/APAD/dl/" + dlInfo.fileName;
									...
									flag = true;
									Util.saveDownloadPath(context, path + "/APAD/dl");
					...
					if (!flag && !dlInfo.filePath.contains(Environment.getDataDirectory().getPath())
						...
						if (needSpace < memoryFreeStorage) {
							dlInfo.filePath = Environment.getDataDirectory().getAbsolutePath() + "/APAD/dl/" + dlInfo.fileName;
							// 断点续传，有新的存储空间，删除以前的未下载完成的temp文件
							if (dlInfo.currentBytes > 0)
								file.delete();
							file = null;
			...
			if (!flag) {
				bundle.putInt("statuscode", DFException.DOWNLOAD_NO_STORAGE_SPACE);
				return bundle;
			}
			...
			bundle.putInt("statuscode", DFException.DOWNLOAD_DOINBACKGROUND_IO_EXCEPTION);
		}
		return bundle;
	}
	...
	protected void onPostExecute(Bundle result) {
		...
		if (result.containsKey("statuscode")) {
			...
			if (null != listener)
				listener.errorDownload(dlInfo, new DFException(0));
			if (result.getInt("statuscode") == DFException.DOWNLOAD_NO_STORAGE_SPACE)
	...
	public void pause() {
		this.cancel(true);
		ULog.e(TAG, dlInfo.contentId + " -->pause()!");
		dlInfo.status = STATE_PAUSE;
	}
	...
	/** 写入文件 */
	private void write(File file, InputStream ins, Bundle bundle) throws DFException {
		...
		try {
			...
			// 对资源长度异常的处理
			if (dlInfo.currentBytes <= 0) {
				bundle.putInt("statuscode", DFException.DOWNLOAD_CONTENT_LENGTH_ZERO);
				return;
			}
			...
			throw new DFException(DFException.DOWNLOAD_FILE_NOT_FOUND_EXCEPTION);
		} catch (IOException e) {
			throw new DFException(DFException.DOWNLOAD_WRITE_IO_EXCEPTION);
		} finally {
			try {
				if (null != ins) {
					ins.close();
