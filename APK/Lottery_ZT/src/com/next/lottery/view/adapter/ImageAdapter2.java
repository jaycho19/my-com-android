...
public class ImageAdapter2 extends PagerAdapter {
	public static final String		TAG	= ImageAdapter2.class.getSimpleName();
	protected Context				context;
	protected List<KVImageBean>	list;
	private int						mCount;
	private OnClickTypeListener		onClickTypeListener;
	// private int mPosition;
	//private ImageGallery			imageGallery;

	public ImageAdapter2(Context context, ArrayList<KVImageBean> listFromDetail,OnClickTypeListener onClickTypeListener) {
		this.context = context;
		this.list = listFromDetail;
		mCount = listFromDetail.size();
...
/*	public ImageAdapter1(Context context, List<RecommendData> list, ImageGallery imageGallery) {
	...
	public Object instantiateItem(ViewGroup container, int position) {
		...
		// 加载网络图片
		bitmapUtils.display(fling_image, list.get(position).getPath());
